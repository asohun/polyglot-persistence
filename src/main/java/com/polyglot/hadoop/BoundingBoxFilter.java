...
public class BoundingBoxFilter extends WritableByteArrayComparable {
	...
	private BoundingBox stringToBB(String v) throws Exception {
		...
		BoundingBox bb = new BoundingBox(lowerLeftLat, upperRightLat,
				lowerLeftLon, upperRightLon);
		System.out.println(" Bounding box is " + "("
				+ bb.getLowerLeft().getLatitude() + ","
				+ bb.getLowerLeft().getLatitude() + ")" + "("
				+ bb.getUpperRight().getLatitude() + ","
				+ bb.getUpperRight().getLatitude() + ")");
		return bb;
	}
	...
	private Point bytesToPoint(byte[] bytes) {
		...
		System.out.println(" Point is " + "(" + p.getLatitude() + ","
				+ p.getLatitude() + ")");
		return p;
	}
